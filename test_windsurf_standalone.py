import logging
import os
import sys
import traceback
from pathlib import Path

from langchain_core.messages import HumanMessage, SystemMessage

import providers.windsurf as windsurf_module

logger = logging.getLogger(Path(__file__).stem)
logger.setLevel(logging.INFO)
handler = logging.StreamHandler(sys.stdout)
handler.setFormatter(logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s"))
logger.addHandler(handler)
#!/usr/bin/env python3
"""
Simple test for Windsurf provider, importing directly to avoid copilot issues.
"""
# Set up logger
logger = logging.getLogger(Path(__file__).stem)
logger.setLevel(logging.INFO)
handler = logging.StreamHandler(sys.stdout)
handler.setFormatter(logging.Formatter("%(asctime)s - %(name)s - %(levelname)s - %(message)s"))
logger.addHandler(handler)
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
# Set environment variables for Windsurf
os.environ["WINDSURF_API_KEY"] = (
    "sk-ws-01-GBW0muKg37rVCHFNsbELDH3nQh3XoscPUzngD-JBg4Y9h1JmnMoV3dKtAee78w6HL_rN_CDf6Cr2uBJjxthKBYLWPctH7g"
)
os.environ["WINDSURF_LS_PORT"] = "54336"
os.environ["WINDSURF_LS_CSRF"] = "da9755d1-18ce-4087-b94c-3cc4e6b67f08"
os.environ["WINDSURF_MODEL"] = "deepseek-v3"  # Free model
try:
    # Import windsurf module directly to avoid __init__.py issues
    WindsurfLLM = windsurf_module.WindsurfLLM
    logger.info("Testing Windsurf provider with free model...")
    # Force cascade mode
    os.environ["WINDSURF_MODE"] = "cascade"
    llm = WindsurfLLM()
    logger.info(f"Initialized with mode: {llm._mode}, model: {llm.model_name}")
    # Simple test message
    messages = [
        SystemMessage(content="You are helpful. Answer briefly."),
        HumanMessage(content="Hello, what is 2+2?"),
    ]
    logger.info("Sending test message...")
    result = llm.invoke(messages)
    logger.info(f"Response: {result.content}")
    logger.info("✓ Windsurf provider works!")
except Exception as e:
    logger.info(f"✗ Error: {e}")
    traceback.print_exc()
