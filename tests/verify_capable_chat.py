...
async def test_persona_intelligence():
    ...
    print(f"Deep Persona Flag: {analysis.get('use_deep_persona')}")
    response = await atlas.chat(
        "Розкажи про свою місію та хто тебе створив?",
    ...
    if "orchestrator" in response.lower() and (
        "клас" in response.lower() or "loop" in response.lower() or "trinity" in response.lower()
    ):
    ...
    query = (
        "Перевір мій календар на сьогодні та знайди файли з назвою 'config' за допомогою Spotlight."
    )
    analysis = await atlas.analyze_request(query)
    print(f"Intent detected: {analysis.get('intent')}")
    response = await atlas.chat(query)
    ...
    if (
        "календар" in response.lower()
        or "config" in response.lower()
        or "spotlight" in response.lower()
    ):
