...
async def main():
    ...
    try:
        ...
        rid = await send_request(
            "tools/call",
            {"name": "macos-use_fetch_url", "arguments": {"url": "https://example.com"}},
        )
        ...
        # Test Time
        print("\n[Test 2] Getting Time...")
        rid = await send_request("tools/call", {"name": "macos-use_get_time", "arguments": {}})
        while True:
            line = await process.stdout.readline()
            try:
                resp = json.loads(line.decode())
        ...
        rid = await send_request(
            "tools/call",
            {"name": "macos-use_run_applescript", "arguments": {"script": 'return "Hello Native"'}},
        )
        while True:
            line = await process.stdout.readline()
            try:
