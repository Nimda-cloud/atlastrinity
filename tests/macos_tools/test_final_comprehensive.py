...
async def final_comprehensive_test():
    ...
    try:
        # Test with template
        result = await manager.call_tool('macos-use', 'macos-use_send_notification', {
            'template': 'reminder',
            'sound': 'default',
            'persistent': True
        })
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_notification_schedule', {'list': True})
        if result and hasattr(result, 'content') and result.content:
            content = result.content[0].text if result.content else ''
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_window_management', {
            'action': 'snapshot',
            'snapshotPath': '/tmp/final_window_test.png'
        })
        if result and hasattr(result, 'content') and result.content:
            content = result.content[0].text if result.content else ''
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_take_screenshot', {
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_analyze_screen', {
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_system_control', {
            'action': 'get_info'
        })
        if result and hasattr(result, 'content') and result.content:
            content = result.content[0].text if result.content else ''
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_create_event', {
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_mail_send', {
            'to': 'test@example.com',
    ...
    try:
        result = await manager.call_tool('macos-use', 'macos-use_finder_list_files', {
            'path': '/tmp',
            'filter': '*.txt',
            'sort': 'name',
            'limit': 5,
    ...
    try:
        ...
        if result and hasattr(result, 'content') and result.content:
            ...
            if apps and len(apps) > 0:
                first_app = apps[0]
                print(f"✅ Enhanced apps: PID={first_app.get('pid')}, Status={first_app.get('processStatus')}")
                success_count += 1
                enhancement_count += 1
            else:
