...
ALL_TOOLS_SCENARIOS = [
    ...
    (
        "macos-use_mail_send",
        {"to": "test@example.com", "subject": "MCP Test", "body": "Test email"},
        "Send test email",
    ),
    # AppleScript (2)
    (
        "macos-use_run_applescript",
        {"script": 'return "AppleScript test successful"'},
        "Run AppleScript",
    ),
    (
        "macos-use_run_javascript",
        {"script": 'return "JavaScript test successful"'},
        "Run JavaScript",
    ),
    # System Control (4)
    ...
    (
        "macos-use_move_file",
        {"source": str(TEST_DIR / "test_move.txt"), "destination": str(TEST_DIR / "moved.txt")},
        "Move file",
    ),
    (
        "macos-use_copy_file",
        {"source": str(TEST_DIR / "test_copy.txt"), "destination": str(TEST_DIR / "copied.txt")},
        "Copy file",
    ),
    # Advanced (4)
    ...
    (
        ...
        {"title": "MCP Test", "message": "Testing all 42 tools"},
        "Show notification",
    ),
    (
        "macos-use_speak_text",
        {"text": "All 42 tools test completed successfully"},
...
async def main():
    ...
    results = {"success": 0, "error": 0, "timeout": 0, "total": len(ALL_TOOLS_SCENARIOS)}
