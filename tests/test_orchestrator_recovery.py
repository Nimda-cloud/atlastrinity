...
async def test_recovery_uses_grisha_and_announces_grisha_message(monkeypatch):
    ...
    async def fake_execute_step(step_copy, attempt=1):
        return StepResult(step_id=step_copy.get("id", 1), success=False, result="failed", error="simulated")
    ...
    async def fake_verify_step(step, result, screenshot_path=None, goal_context="", task_id=None):
        return VerificationResult(step_id=step.get("id"), verified=False, confidence=0.1, description="Bad", issues=["issue1"], voice_message="GRISHA_MESSAGE")
