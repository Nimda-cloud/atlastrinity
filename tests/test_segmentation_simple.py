...
def test_keyword_segmentation():
    ...
    # Test mixed request
    user_request = (
        "Привіт! Як справи? Створи React компонент для калькулятора і знайди погоду у Львові"
    )
...
def test_segment_profile_building():
    ...
    print(
        f"Dev profile - Mode: {dev_profile.mode}, Tools: {dev_profile.tools_access}, Vibe: {dev_profile.use_vibe}"
    )

    assert dev_profile.mode == "development"
    assert dev_profile.tools_access == "full"
    assert dev_profile.use_vibe
