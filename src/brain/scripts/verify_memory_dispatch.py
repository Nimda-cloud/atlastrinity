...
async def verify():
    ...
    try:
        ...
        res = await mcp_manager.dispatch_tool("memory", {"query": "Atlas Test Dispatch", "limit": 1, "step_id": "test_123"}, explicit_server="memory")
        
        print(f"Result Type: {type(res)}")
        
        # Check if result looks like search results
        res_str = str(res)
        if "results" in res_str or "entities" in res_str or "Atlas" in res_str or isinstance(res, list) or isinstance(res, dict):
