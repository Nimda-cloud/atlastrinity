...
class ProcessWatchdog:
                        ...
                        if pid not in self.processes:
                            ...
                            try:
                                ...
                                self.processes[pid] = {
                                    ...
                                    "started": datetime.fromtimestamp(child.create_time()).isoformat(),
                                    "type": proc_type,
                                    "last_seen": time.time(),
                                    "stuck_count": 0,
                                    "cpu_history": [],
                                ...
                                logger.info(f"[WATCHDOG] New process tracked: {proc_type} (PID: {pid}, Name: {proc_name})")
    ...
    def _classify_process(self, proc: psutil.Process) -> str:
        try:
            ...
            if "vibe_server" in cmd: return "mcp_vibe"
            if "src.mcp_server" in cmd: return "mcp_server"
            if "devtools_server" in cmd: return "mcp_devtools"
            if "vibe" in cmd and "-p" in cmd: return "vibe_cli"
            if "copilot_proxy" in cmd: return "proxy"
            if "npx" in cmd or "node" in cmd: return "mcp_external"
            if "python" in cmd: return "python_sub"
            return "unknown"
                ...
                try:
                    ...
                    if info["type"] == "vibe_cli":
                        ...
                        if info["stuck_count"] >= 12:
                            logger.warning(f"[WATCHDOG] Process {pid} ({info['type']}) seems STUCK (0% CPU for long time).")
                    ...
                    if info["memory_mb"] > 2048: # > 2GB
                        logger.warning(f"[WATCHDOG] Process {pid} ({info['type']}) high memory: {info['memory_mb']:.1f} MB")
                        self.mon.log_for_grafana(f"High memory process detected: {pid}", level="warning", memory=info["memory_mb"])
    ...
    async def handle_stuck_process(self, pid: int):
        ...
        logger.error(f"[WATCHDOG] 🚨 AUTO-HEALING: Terminating stuck process {pid} ({info['type']})")
