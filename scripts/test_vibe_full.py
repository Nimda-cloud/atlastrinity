...
async def run_test():
    ...
    config_res = await vibe_server.vibe_get_config(ctx)
    ...
    # Clean previous run
    if os.path.exists("fib_app.py"): os.remove("fib_app.py")
    if os.path.exists("test_fib_app.py"): os.remove("test_fib_app.py")

    impl_res = await vibe_server.vibe_implement_feature(
        ctx,
    ...
    if os.path.exists("fib_app.py"):
        review_res = await vibe_server.vibe_code_review(ctx, "fib_app.py", focus_areas="performance")
    ...
    if os.path.exists("test_fib_app.py"):
        ...
        with open("fib_app.py") as f: fib_code = f.read()
        with open("test_fib_app.py") as f: test_code = f.read()
        
        sandbox_res = await vibe_server.vibe_test_in_sandbox(
            ctx,
    ...
    logger.info("\n7. Testing vibe_ask...")
    ask_res = await vibe_server.vibe_ask(ctx, "What is the time complexity of a recursive fibonacci function?", model="gpt-4o")
