...
TEST_CASES = {
    ...
    "duckduckgo-search": ("duckduckgo_search", {"query": "Model Context Protocol", "max_results": 1}),
...
async def run_mcp_tool(server_name: str, config: Dict[str, Any], tool_name: str, tool_args: Dict[str, Any]) -> bool:
    ...
    try:
        if process.stdin:
            ...
            # For chrome-devtools, we might not know a valid tool name without listing first if 'list_targets' is wrong.
            # But let's try the requested tool first.
            ...
            while True:
                ...
                try:
                    ...
                    if msg.get("id") == 2:
                        if "error" in msg:
                            # If tool not found, user might have wanted us to test something else.
                            # But generally error response means the SERVER is working (it processed the request).
                            if msg['error'].get('code') == -32601: # Method/Tool not found
                                print(f"⚠️ Tool not found: {tool_name}. But server responded!")
                                success = True # Partial success - server is alive
                            else:
                                print(f"❌ Error: {msg['error']}")
                                # For whisper, if audio file is empty/invalid, it might error but that means server is running.
                                if server_name == "whisper-stt": success = True 
                        else:
