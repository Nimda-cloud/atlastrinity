...
class SecureBackupManager:
    ...
    def __init__(self, project_root: Path):
        ...
        self.exclude_files = {
            ...
            'config.yaml', 'behavior_config.yaml', 
            'mcp_servers.json', 'vibe_config.toml'
        }
    ...
    def decrypt_file(self, input_path: Path, output_path: Path, key: bytes) -> bool:
        ...
        try:
            ...
            print(f"❌ Failed to decrypt {input_path.name}: {e}")
            return False
        ...
        try:
            ...
            for table in tables:
                ...
                if clean_rows:
                    ...
                    clean_conn.executemany(f"INSERT INTO {table} VALUES ({placeholders})", clean_rows)
            ...
            print(f"❌ Failed to filter SQLite secrets from {db_path.name}: {e}")
            return False
    ...
    def create_secure_backup(self) -> bool:
        ...
        print("🔒 Creating secure backup with encryption...")
        ...
        for mapping in backup_mappings:
            ...
            try:
                ...
                if source.is_file():
                        ...
                        if self.filter_sqlite_secrets(source, temp_filtered):
                            if mapping.get('encrypt', False):
                                if self.encrypt_file(temp_filtered, dest, key):
                                    print(f"✅ Filtered & encrypted: {source.name}")
                                    success_count += 1
                                else:
                                    print(f"❌ Encryption failed: {source.name}")
                            else:
                                shutil.move(temp_filtered, dest)
                                print(f"✅ Filtered: {source.name}")
                                success_count += 1
                            # Clean up temp file
                            temp_filtered.unlink(missing_ok=True)
                        else:
                            print(f"❌ Filtering failed: {source.name}")
                    elif mapping.get('encrypt', False):
                        if self.encrypt_file(source, dest, key):
                            print(f"✅ Encrypted: {source.name}")
                            success_count += 1
                        else:
                            print(f"❌ Encryption failed: {source.name}")
                    else:
                    ...
                    print(f"✅ Directory copied: {source.name}")
                    success_count += 1
        ...
        print(f"🔒 Secure backup completed: {success_count} items processed")
        print(f"📋 Metadata saved: {metadata_path}")
        return True
    
    def restore_secure_backup(self) -> bool:
        """Restore from secure backup with decryption"""
        print("🔓 Restoring from secure backup...")
                ...
                if source.is_file():
                    if mapping.get('encrypted', False):
                        ...
                        if self.decrypt_file(source, temp_decrypted, key):
                            ...
                            print(f"✅ Decrypted & restored: {dest.name}")
                            success_count += 1
                        else:
                            print(f"❌ Decryption failed: {source.name}")
                    ...
                    if dest.exists():
                        shutil.rmtree(dest)
                    shutil.copytree(source, dest)
                    print(f"✅ Directory restored: {dest.name}")
                    success_count += 1
                    
            except Exception as e:
                print(f"❌ Restore failed for {source.name}: {e}")
