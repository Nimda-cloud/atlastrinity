...
async def test_server_tools(name: str, cfg: dict, tool_args: dict):
    ...
    for tool in tools:
        ...
        test_params = tool_args.get(tool_name)
...
async def main():
    ...
    print(f"{'=' * 70}")
    print(f" COMPREHENSIVE MCP RUNTIME TEST — {len(active)} servers, 289 tools")
    print(f"{'=' * 70}\n")
    sys.stdout.flush()
    ...
    for name, cfg in active:
        ...
        for r in results:
            ...
            t = f" [{r.get('time', 0):.1f}s]" if r.get("time") else ""
            print(f"  {icon} {r['tool']}{t}{note}")
        sys.stdout.flush()

        print(
            f"  → {name}: {passed}/{total} PASS"
    ...
    print(" FINAL SUMMARY")
    print(f"{'=' * 70}")
    print(f" PASS:  {total_pass}/{total_all}")
    print(f" SKIP:  {total_skip}/{total_all}")
    print(f" FAIL:  {total_fail}/{total_all}")
    print(f"{'=' * 70}")
    sys.stdout.flush()
    ...
    if total_fail > 0:
        ...
        for r in all_results:
            if r["status"] not in ("PASS", "SKIP"):
                print(f"  ✗ [{r['server']}] {r['tool']}: {r['status']} — {r.get('error', '')}")
        sys.stdout.flush()

    # Save results
    Path("/tmp/mcp_runtime_results.json").write_text(json.dumps(all_results, indent=2, default=str))
    print("\nResults saved to /tmp/mcp_runtime_results.json")
