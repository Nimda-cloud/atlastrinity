    ...
    async def call_macos_tool(self, tool_name: str, params: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                self.stats["errors"] += 1
                return {"status": "error", "tool": tool_name, "error": stderr or "Unknown error"}

        except subprocess.TimeoutExpired:
            self.stats["errors"] += 1
            return {"status": "timeout", "tool": tool_name, "error": "Tool call timed out"}
...
class CompleteXcodeBuildMCPIntegration:
        ...
        for suite in test_suites:
            ...
            for tool in suite["tools"]:
                ...
                suite_results.append(
                    {"tool": tool, "status": result.get("status"), "result": result}
                )
    ...
    def generate_complete_report(self, all_results):
        ...
        for suite in all_results:
            ...
            print(
                f"   {status} {suite['suite']}: {suite_success}/{suite_total} ({suite_rate:.1f}%)"
            )
            print(f"      Impact: {suite['impact']}")
