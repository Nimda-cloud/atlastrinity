...
def validate_server(name: str, config: dict) -> dict:
    ...
    elif "mcp-server-macos-use" in command or command.endswith(".build/release/mcp-server-macos-use"):
        result["type"] = "Swift"
...
def main():
    ...
    for r in sorted(results, key=lambda x: (x.get("tier", 99), x["name"])):
        ...
        status_color = Colors.OKGREEN if r["status"] == "ok" else (Colors.FAIL if r["status"] == "error" else Colors.WARNING)
        
        print(f"{r['name']:<25} {r['type']:<10} {r['tier']:<6} {status_color}{status_icon} {r['status']}{Colors.ENDC}")
