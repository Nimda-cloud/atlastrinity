...
async def test_vibe_prompt_fallback():
    ...
    with patch("mcp_server.vibe_server.is_network_available", return_value=True):
        ...
        config.providers = [
            ProviderConfig(name="mistral", api_base="http://fail", api_key_env_var="FAIL_MISTRAL"),
            ProviderConfig(name="openrouter", api_base="http://fail", api_key_env_var="FAIL_OPENROUTER"),
            ProviderConfig(name="copilot", api_base="http://127.0.0.1:8085", api_key_env_var="COPILOT_SESSION_TOKEN")
        ]
