...
async def test_tool(mcp_manager, tool_name, arguments):
    ...
    try:
        ...
        # Check success
        success = False
        if (isinstance(result, dict) and result.get("success")) or (hasattr(result, "is_error") and not result.is_error) or (isinstance(result, list) and len(result) > 0):
            success = True
...
async def main():
    ...
    test_cases = [
        ...
        # --- Notifications ---
        ("macos-use_send_notification", {"title": "Test Notification", "message": "Atlas Trinity Testing Phase 8"}),
        ...
        # --- Personal Info ---
        ("macos-use_calendar_events", {"start": "2026-01-01T00:00:00Z", "end": "2026-12-31T23:59:59Z"}),
