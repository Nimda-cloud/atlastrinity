...
async def verify_integrity():
    ...
    for server_name, server_cfg in mcp_config.items():
        ...
        print(f"Checking {Colors.BOLD}{server_name}{Colors.ENDC}...")
        ...
        try:
            ...
            for tool in live_tools:
                ...
                if not static_schema:
                    ...
                    if tool_name in key_tools:
                        schema_issues.append(f"Tool {tool_name} is a KEY tool but has NO schema in tool_schemas.json")
                    continue
                
                # Check server ownership or alias
                if static_schema.get("server") != server_name and "alias_for" not in static_schema:
                    schema_issues.append(f"Tool {tool_name} assigned to server {static_schema.get('server')} but found in {server_name}")
                ...
                if missing_required:
                    schema_issues.append(f"Tool {tool_name}: Missing REQUIRED args in static config: {missing_required}")
                ...
                if extra_static_args:
                    schema_issues.append(f"Tool {tool_name}: Static config contains EXTRA/INVALID args: {extra_static_args}")
            ...
            if status == "OK":
                print(f"  {Colors.GREEN}✓ Integrity Confirmed ({len(live_tool_names)} tools){Colors.ENDC}")
            else:
                if missing_key_tools:
                    print(f"  {Colors.RED}✗ Missing Key Tools: {missing_key_tools}{Colors.ENDC}")
                if schema_issues:
                    print(f"  {Colors.YELLOW}⚠ Schema Discrepancies: {len(schema_issues)}{Colors.ENDC}")
                    for issue in schema_issues:
                        print(f"    - {issue}")
                        
        except Exception as e:
            print(f"  {Colors.RED}✗ Connection Failed: {str(e)[:100]}{Colors.ENDC}")
            report.append({
                "server": server_name,
    ...
    for item in report:
        s_color = Colors.GREEN if item["status"] == "OK" else (Colors.RED if item["status"] == "CONNECTION FAILED" else Colors.YELLOW)
