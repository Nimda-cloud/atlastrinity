...
Usage:
    python scripts/copilot_vibe_proxy.py                    # Default port 8086
    python scripts/copilot_vibe_proxy.py --port 8087       # Custom port
...
try:
    ...
    print(f"Error importing CopilotLLM: {e}")
    print("Please ensure providers.copilot is available")
    sys.exit(1)
...
def log(msg: str) -> None:
    ...
    print(f"[{timestamp}] {C.CYAN}[CopilotProxy]{C.RESET} {msg}")

def info(msg: str) -> None:
    timestamp = time.strftime("%H:%M:%S")
    print(f"[{timestamp}] {C.GREEN}[CopilotProxy]{C.RESET} {msg}")

def warn(msg: str) -> None:
    timestamp = time.strftime("%H:%M:%S")
    print(f"[{timestamp}] {C.YELLOW}[CopilotProxy]{C.RESET} {msg}")

def error(msg: str) -> None:
    timestamp = time.strftime("%H:%M:%S")
    print(f"[{timestamp}] {C.RED}[CopilotProxy]{C.RESET} {msg}")
...
def run(port: int = DEFAULT_PORT) -> None:
    ...
    if not api_key:
        ...
        error("Run: python -m providers.get_copilot_token --method vscode --update-env")
        sys.exit(1)
